...
from functions import mylamb2g0, band_1dsquare_lattice
import os
import time
...
class Migdal(MigdalBase):
    ...
    def compute_S(self, G, D):
        return -self.g0**2/self.nk * conv(G, D, ['k-q,q'], [0], [True], self.beta)
    #------------------------------------------------------------
    def compute_GG(self, G):
        return 2.0/self.nk * conv(G, -G[:,::-1], ['k,k+q'], [0], [True], self.beta)
    #------------------------------------------------------------
