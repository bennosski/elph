...
class Migdal(MigdalBase):
    ...
    def compute_G(self, wn, eks, mu, S):
        ek, ekpq = eks
        G = linalg.inv(1j*wn[None,:,None,None]*Migdal.tau0[None,None,:,:] - (ek[:,None,None,None]-mu)*Migdal.I00[None,None,:,:] - (ekpq[:,None,None,None]-mu)*Migdal.I11[None,None,:,:] - S)
        return G         
      
        #return linalg.inv(1j*wn[None,:,None,None]*Migdal.tau0[None,None,:,:] - (ek[:,None,None,None]-mu)*Migdal.I00[None,None,:,:] - (ekpq[:,None,None,None]-mu)*Migdal.I11[None,None,:,:] - S)
    #------------------------------------------------------------
    def compute_D(self, vn, PI):
        #return 1.0/(-((vn**2)[None,:] + self.omega**2)/(2.0*self.omega) - PI)

        return linalg.inv(-((vn**2)[None,:,None,None] + self.omega**2)/(2.0*self.omega)*Migdal.tau0[None,None,:,:] - PI)
    #------------------------------------------------------------
    def compute_S(self, G, D):
        #tau3Gtau3 = einsum('ab,kwbc,cd->kwad', Migdal.tau3, G, Migdal.tau3)
        return -self.g0**2/self.nk * conv(G, D, ['k-q,q'], [0], [True], beta=self.beta, op='...ab,...bc->...ac')
    #------------------------------------------------------------
    def compute_GG(self, G):
        ...
        return 2.0/self.nk * conv(G, -G[:,::-1], ['k,k+q'], [0], [True], beta=self.beta, op='...ab,...bc->...ac')
    #------------------------------------------------------------
    def init_selfenergies(self):
        S  = zeros([self.nk,self.ntau,2,2], dtype=complex)
        PI = zeros([self.nk,self.ntau,2,2], dtype=complex)
        if self.sc:
           S[...,0,0,1]  = 0.01
           S[...,0,1,0]  = 0.01
           PI[...,0,0,1] = 0.01
