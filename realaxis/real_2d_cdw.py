import imagaxis
from migdal_2d_cdw import Migdal
...
class RealAxisMigdal(Migdal):
    ...
    def setup_realaxis(self):
        ...
        ek = self.band(self.nk, 1.0, self.tp, self.Q)
        
        return wn, vn, ek, w, nB, nF, DRbareinv
    #----------------------------------------------------------- 
    def compute_GR(self, w, eks, mu, SR):
        ek, ekpq = eks
        ginv = (w[None,None,:,None,None]+self.idelta)*Migdal.tau0[None,None,None,:,:] - (ek - mu)[:,:,None,None,None]*Migdal.I00[None,None,None,:,:] - (ekpq - mu)[:,:,None,None,None]*Migdal.I11[None,None,None,:,:] - SR
    ...
    def compute_SR(self, GR, Gsum, DR, nB, nF):
        ...
        return -self.g0**2*self.dw/self.nk**2 * ( \
             ...
             -basic_conv(B*(1+nB[None,None,:,None,None]), GR, ['q,k-q','q,k-q','z,w-z'], [0,1,2], [True,True,False])[:,:,:self.nr] \
             +basic_conv(B, GR*nF[None,None,:,None,None], ['q,k-q','q,k-q','z,w-z'], [0,1,2], [True,True,False])[:,:,:self.nr])
    ...
    def compute_PIR(self, GR, Gsum, nF):
        ...
        return self.g0**2*self.dw/self.nk**2 * np.einsum('...aa->...', \
           basic_conv(A, Gsum, ['k+q,k','k+q,k','z,w-z'], [0,1,2], [True,True,False], op='...ab,...bc->...ac')[:,:,:self.nr] \
          -basic_conv(A, GA*nF[None,None,:,None,None], ['k+q,k','k+q,k','w+z,z'], [0,1,2], [True,True,False], op='...ab,...bc->...ac')[:,:,:self.nr] \
          +basic_conv(A*nF[None,None,:,None,None], GA, ['k+q,k','k+q,k','w+z,z'], [0,1,2], [True,True,False], op='...ab,...bc->...ac')[:,:,:self.nr])
